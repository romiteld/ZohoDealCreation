-- GPT-5 400K Context Support Migration for Azure Cosmos DB PostgreSQL
-- Simplified version without unsupported features

-- Enable pgvector extension
CREATE EXTENSION IF NOT EXISTS vector;

-- 1. Email Contexts Table (400K token support)
...
-- Vector similarity indexes (HNSW for accuracy)
CREATE INDEX IF NOT EXISTS idx_context_embedding_hnsw 
ON email_contexts_400k USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS idx_pattern_embedding_hnsw 
ON correction_patterns_v2 USING hnsw (pattern_embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Simple views for analytics (no materialized views in Cosmos DB)
...
-- Note: Cosmos DB PostgreSQL handles permissions differently
-- These are placeholders - adjust based on your actual users
