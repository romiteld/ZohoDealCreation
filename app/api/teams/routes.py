...
async def handle_message_activity(
    ...
    if any(greeting in message_text for greeting in ["hello", "hi", "hey"]):
        ...
        normalized_command = _normalize_command(message_text)
        ...
        elif normalized_command.startswith("preferences"):
            return await show_user_preferences(user_id, user_email, user_name, db)

        elif normalized_command.startswith("analytics"):
...
async def generate_digest_preview(
    ...
    try:
        ...
        if test_recipient_email:
            logger.info(
                "Digest preview running in test mode for %s", test_recipient_email
            )
