"""
Optimized CrewAI manager using Cosmos DB for PostgreSQL instead of Chroma
...
# NO CHROMA/SQLITE IMPORTS - Using Cosmos DB instead!
...
class EmailProcessingCrew:
    """Email processing crew using Cosmos DB for context storage"""
    ...
    @property
    def llm(self):
        """Lazy load LLM with proper initialization"""
        if not self._llm:
            try:
                ...
                if not api_key:
                    ...
                    self._llm = "gpt-5-mini"  # Fallback for CrewAI
                else:
                    # Initialize ChatOpenAI directly to avoid proxy issues
                    ...
                    logger.info("LLM initialized with ChatOpenAI")
            except Exception as e:
                logger.warning(f"Failed to initialize ChatOpenAI: {e}, using string fallback")
    ...
    def _initialize_crew(self):
        ...
        try:
            ...
            logger.info("CrewAI agents initialized successfully (Cosmos DB mode)")
    ...
    def _create_crew(self, email_content: str, sender_domain: str):
        ...
        # Create optimized crew - NO MEMORY (no Chroma needed!)
        return Crew(
            agents=agents,
            tasks=tasks,
            process=Process.sequential,
            verbose=False,  # Reduce output
            memory=False,  # NO MEMORY - No Chroma/SQLite needed!
    ...
    async def run_async(self, email_body: str, sender_domain: str) -> ExtractedData:
        """Async run with Cosmos DB caching"""
        # Initialize vector store
        await self._init_vector_store()
        
        # Check cache first if vector store is available
        if self._vector_store:
            cached_result = await self._vector_store.get_cached_result(email_body)
            if cached_result:
                logger.info("Returning cached result from Cosmos DB")
                return ExtractedData(**cached_result)
        ...
        if self._vector_store and result:
            await self._vector_store.cache_email_result(
                email_body,
                result.model_dump()
            )
    ...
    def run(self, email_body: str, sender_domain: str) -> ExtractedData:
        ...
        try:
            ...
            return SimplifiedEmailExtractor.extract(email_body, sender_domain)
